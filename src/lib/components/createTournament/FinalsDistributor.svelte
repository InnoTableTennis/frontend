...
{#await countGroups() then}
    <div class="content">
        {#each finals as final, i}
            <div class="separator" class:invisible={peopleInGroups <= 8 || numberFinals <= 1} class:invisible-small={peopleInGroups <= 4 || numberFinals <= 1}></div>
            <div class="line">
                <h2>
                    {stringifyNumber(i+1).charAt(0).toUpperCase() + stringifyNumber(i+1).slice(1)} final
                </h2>
                <div class="buttons" class:not-fit={peopleInGroups > 8} class:not-fit-small={peopleInGroups > 4} >
                    {#each groups as j}
                        <button
                            class="button"
